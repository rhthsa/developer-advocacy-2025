...
* Review the Instrumentation configurations here https://github.com/rhthsa/developer-advocacy-2025/blob/main/config/otel/instrumentation.yaml. You can see the Otel Collector endpoint and a bunch of custom configurations for the Java and Go application auto-instrumentation.
+
...
* Copy all YAML code from this https://github.com/rhthsa/developer-advocacy-2025/blob/main/config/otel/instrumentation.yaml and paste to YAML editor in OpenShift console. Then click *Create* button.
+
image::otel/otel-15.png[]

* The `instrumentation` resource should be created.
+
image::otel/otel-16.png[]

[#deployappforautoinstrument]
== Deploy Appliations for Auto-Instrumentation

In this lab, we're going to deploy 3 applications developed with different programming languages and frameworks to demonstrate Open Telemetry auto-instrumentation. Basically, you'll be able to trace transactions in your applications without changing the codes to add dependency or library to your application.

=== Deploy `frontend` Node.js Application.

* Review deployment configuration for the `frontend` application here: https://github.com/voraviz/openshift-otel/blob/main/config/frontend.yaml
+
image::otel/otel-10.png[]

* Open the Web Terminal in OpenShift console, then run these commands to deploy the `frontend` application.
+
IMPORTANT: Don't forget to change *`user-X`* in the command to your username.
+
[.console-input]
[source,adoc]
----
...
* Run this command to update the `frontend` Deployment resource by adding an annotation to the Deployment for auto-instrumentation.
+
[.console-input]
[source,adoc]
----
...
* Run this command to verify that auto-instrumentation is working as init-container.
+
[.console-input]
[source,adoc]
----
...
* Run these commands to set environment variables for the `frontend` Deployment. These environment variables will be used by OTel auto-instrumentation.
+
[.console-input]
[source,adoc]
----
...
=== Deploy `simple-go` Go Application.

NOTE: With opentelemetry-operator v0.119.0-2 and Go autoinstrumentation-go:v0.20.0 only work with golang 1.23

* Review deployment configuration for the `simple-go` application here: https://github.com/voraviz/openshift-otel/blob/main/config/simple-go.yaml
+
image::otel/otel-11.png[]

* For the `frontend` application, we added annotation and set environment variables with `oc patch` and `oc set env` commands to the Deployment after we've created it .
+
But this time, we'll add the annotations and environment variables to the `simple-go` Deployment YAML configurations before we create the Deployment resource to demonstate different ways to configure OTel auto-instrument for Deployment resource.
+
Annotations in `simple-go` Deployment YAML configurations.
+
...
----
+
Enviornment variables in `simple-go` Deployment YAML configurations.
+
[source,yaml,subs="+macros,attributes+"]
----
...
* Run this command to deploy `simple-go` application.
+
[.console-input]
[source,adoc]
----
...
* Run this command to verify that `simple-go` application Pod status is `Running`.
+
[.console-input]
[source,adoc]
----
...
* Run this command to verify that auto-instrumentation is working with as sidecar container.
+
[.console-input]
[source,adoc]
----
...
* Review deployment configuration for the `backend` application here: https://github.com/voraviz/openshift-otel/blob/main/config/backend.yaml
+
image::otel/otel-12.png[]

* The `backend` Deployment configuration already has the annotation and environment variables requried for OTel auto-instrumentation.
+
Annotations.
+
[source,yaml,subs="+macros,attributes+"]
...
* Run this command to deploy `backend` application,
+
[.console-input]
[source,adoc]
----
...
* Verify that auto-instrumentation is working as init-container.
+
[.console-input]
[source,adoc]
----
...
* Run this command to check application container's logs. Look for `JAVA_TOOL_OPTIONS` in the logs, you should see the Otel auto-instrumentation Java agent is configured.
+
[.console-input]
[source,adoc]
----
...
* Switch to *Developer* perspective, then select *Topology* menu on the left. All applications should be up and running.
+
...
* Run this command a few times in web terminal to trigger API calls from `frontend` application.
+
IMPORTANT: Don't forget to change `userX` in the command to your username.
+
[.console-input]
[source,adoc]
----
...
* Switch to *Administrator* perspective. Then go to *Observe* -> *Trace* menu on the left. Select  `userX-otel/sample` in *Tempo Instance* drop-down list, select `dev` in *Tenant* drop-down list, and enter `{rootServiceName="frontend"}` to *Query* input. Then click *Run query* button.
+
image::otel/otel-22.png[]

* Click on the trace returned from the query to see trace details.
+
image::otel/otel-23.png[]

* Click on each span to see span details.
+
