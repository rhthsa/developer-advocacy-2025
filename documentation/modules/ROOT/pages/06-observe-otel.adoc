...
* Go to *Topology* menu, click on `tempo-sample` entity, go to the *Resources* tab in the right panel then wait unti the `tempo-sample-0` Pod status changes to `Running`.
+
...
* The `OpenTelemetryCollectors` should be created.
+
...
In this lab, we'll deploy a simple Todo Java application hosted in Github here: The Todo application Source Code is Git repository here: https://github.com/voraviz/quarkus-todo-app/tree/otel.
...
* Review deployment configuration here: https://github.com/voraviz/openshift-otel/blob/main/todo-kustomize/overlays/otel/todo.yaml. Look for the `env` field, you should see the Otel Collector endpoint that Todo application will send tracing data to the collector.
+
...
* Deploy the Todo applciation with command line. Don't forget to change `userX` to match to your username. Then wait until all deployment commplete.
+
