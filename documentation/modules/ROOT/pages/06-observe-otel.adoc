...
* Run these commands to set environment variables for the `frontend` Deployment.
+
[.console-input]
[source,adoc]
----
...
+
But this time, we'll add the annotations to the `simple-go` Deployment YAML configurations before we create the Deployment resource to demonstate different ways to configure OTel auto-instrument for Deployment resource.
+
...
* Run this command in web terminal to trigger API calls from `frontend` application.
+
